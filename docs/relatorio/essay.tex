...
Atualmente, o funcionamento da Loja Social assenta num sistema de gestão baseado em registos manuais em papel e em ficheiros Excel, utilizados para acompanhar entradas, saídas e níveis de stock. Embora este método permita o controlo básico dos bens, torna o processo mais moroso, sujeito a erros e dependente de atualizações manuais.
...
Esta secção apresenta os requisitos funcionais \textbf{(RFs)} que descrevem as funcionalidades a implementar no sistema.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|p{12cm}|}
		...
		\hline
		\textbf{RF 06} & O sistema deve permitir agendar recolhas, especificando data. \\
		\hline
		...
		\hline
		\textbf{RF 11} & O sistema deve permitir ao benefeciário submeter e ver estado da candidatura. \\
		\hline
		\textbf{RF 12} & O sistema deve permitir aos administradores rejeitar e aceitar candidaturas de benefeciários. \\
		\hline
		\textbf{RF 13} & O sistema deve permitir fazer perguntas através de um chat de suporte. \\
		\hline
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|p{12cm}|}
		...
		\hline
		\textbf{RNF01} & O sistema deve processar scans e atualizações de stock de forma eficiente. \\
		\hline
		\textbf{RNF02} & A interface de scan de código de barras deve ser intuitiva e rápida, com mínima necessidade de dados manuais. \\
		\hline
		\textbf{RNF03} & O sistema deve funcionar em dispositivos Android. \\
		\hline
		\textbf{RNF04} & Todos os registos de entrada e saída devem ser persistentes e auditáveis. \\
		\hline
		\textbf{RNF05} & A interface deve ser responsiva e adaptável a diferentes tamanhos de ecrã e resoluções de dispositivos Android. \\
		\hline
		\textbf{RNF06} & O sistema deve apresentar mensagens de erro claras e informativas, sem expor informação técnica sensível. \\
		\hline
		\textbf{RNF 07} & O sistema deve permitir o acesso através de autenticação com email e pass. \\
		\hline
		\textbf{RNF 08} & O chat de suporte deve ser em tempo real. \\
		\hline
		\textbf{RNF 09} & A aplicação deve seguir todas as regras de RGPD. \\
		\hline
	\end{tabular}
...
A página web, desenvolvida em Next.js, serve como ponto de acesso público para a comunidade académica, permitindo visualizar o stock da Loja Social, consultar informações sobre campanhas de doação e acompanhar notícias relacionadas. O Next.js fornece renderização server-side quando necessário, garantindo desempenho otimizado e carregamento rápido de conteúdo.
O acesso a áreas reservadas faz uso do Firebase Auth, assegurando a autenticação de utilizadores e o controlo de permissões.

A aplicação móvel, construída em Kotlin para Android, é utilizada pelos Serviços de Ação Social (SAS) para gerir os beneficiários, o inventário de bens, a calendarização de entregas e alertas de validade. A autenticação dos colaboradores também é realizada através do Firebase Auth, garantindo que apenas utilizadores autorizados podem aceder às funcionalidades de gestão.
